...
def cross_validate(args, dataset, cv, lr, w_d, mmt, drop, perm, net_parameters):
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        batch_size=dataset.shape[0], shuffle=True, **kwargs)

    for epoch in range(1, args.epochs + 1):
        lr_decay = lr * np.exp(-0*epoch / args.epochs)
        print("lr: ", lr_decay)
        optimizer = torch.optim.SGD(model.parameters(), lr=lr_decay, momentum=mmt)
        train(model, device, train_loader, optimizer, w_d, epoch)
        model.eval()
        with torch.no_grad():
            for data, target, fileidx in train_loader:
                data, target = data.to(device), target.to(device)
                output = model(data)
                pred = output.argmax(dim=1, keepdim=True)
        print(torch.sum(~(pred[:,0] == target)).item())
    ...
    out = dataset[fileidx][~(pred[:,0] == target)]
        ...
        for w_d in weight_decay:
            for mmt in momentum:
                for drop in drop_array:
                    ...
                    cross_validate(args, dataset, 20, lr, w_d, mmt, drop, perm, net_parameters)
