...
def HEM_one_level(rr, cc, vv, rid, weights):
    ...
    return cluster_id   # 两两节点配对，为同组节点标记聚类id
...
def compute_perm(parents):
    ...
    for parent in parents[::-1]:
        ...
        indices_layer = []
        for i in indices[-1]:
            indices_node = list(np.where(parent == i)[0])   # indices_node是子节点序号列表
            ...
            indices_layer.extend(indices_node)  # indices_layer是一维展开的子节点列表
